...
const ExpenseList = () => {
  ...
  const fetchExpenseHandler = useCallback(async () => {
    const response = await fetch(
      `https://expense-tracker-999-default-rtdb.asia-southeast1.firebasedatabase.app/expenses${email}.json`
    );
    if (!response.ok) {
  ...
  const handleEditExpense = async (event) => {
    ...
    const response = await fetch(
      `https://expense-tracker-999-default-rtdb.asia-southeast1.firebasedatabase.app/expenses${email}/${updatedExpense.id}.json`,
      {
        method: "PUT",
  ...
  const handleDeleteExpense = async () => {
    const response = await fetch(
      `https://expense-tracker-999-default-rtdb.asia-southeast1.firebasedatabase.app/expenses${email}/${expenseToDelete.id}.json`,
      {
        method: "DELETE",
      }
    );
